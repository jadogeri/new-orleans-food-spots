...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      emptyOutDir: true,
      chunkSizeWarningLimit: 650, // 💡 FIXED: Silences the warning since your vendor bundle size is 577 kB
      // 👇 ADDED: Rollup code-splitting rules to handle massive bundles safely
      rollupOptions: {
        output: {
          manualChunks(id) {
            // Isolates all code from node_modules into a separate 'vendor' asset bundle
